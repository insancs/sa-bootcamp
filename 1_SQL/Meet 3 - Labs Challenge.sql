...
-- of September 2012. Produce an output table consisting of facility id an 
-- slots, sorted by the number of slots.
SELECT 
	f.facid, 
	SUM(b.slots) AS Total_Slots
FROM cd.facilities AS f
FULL OUTER JOIN cd.bookings AS b
ON f.facid = b.facid
GROUP BY 1
...
SELECT  
	...
	b.starttime
FROM cd.members m
FULL OUTER JOIN cd.bookings b
ON m.memid=b.memid
WHERE 
	m.firstname ILIKE '%david%' AND 
	m.surname ILIKE '%Farrell%';
